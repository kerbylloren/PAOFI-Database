...
npm run package:installer
Get-AuthenticodeSignature .\dist-electron\PAOFI-LP-Database-Setup-0.1.4-x64.exe
```

The installer output is:

```text
native-app\dist-electron\PAOFI-LP-Database-Setup-0.1.4-x64.exe
```

The portable standalone build output is:

```text
native-app\dist-electron\PAOFI-LP-Database-Standalone-0.1.4-x64.exe
```

During install/update, the installer removes old generated preview/runtime
folders while preserving Turso config and database data.
